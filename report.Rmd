...
(@) In the code below, we will demonstrate how to apply text cleaning and fuzzy-matching. Note however that the code is not representative of the code actually used to clean our dataframes.
...
## Missing Values
```{r (HIDE) Missing values text, include = FALSE, echo = FALSE}
...
```{r (SHOW) Graduate Prospects Relationships, include = TRUE, echo = TRUE}
...
# Correlation heatmap
ggplot(data = mat_corr, mapping = aes(x = `Var2`, y = `Var1`, fill = value)) +
...
# Section 3: Frequentist Multi-Linear Regression
```{r (HIDE) Frequentist Linear Regression script, include = FALSE, echo = FALSE}
source('3 - analysisFrequentist.R')
```

(@) 

```{r (HIDE) Plot regression diagnostics, echo = FALSE, eval = TRUE}
par(mfrow = c(2, 2))
plot(model_regFreq$finalModel)
rpt_ci
```

```{r (SHOW) Frequentist Linear Regression, echo = TRUE, eval = FALSE}
# Set random number generator seed to a number to reproduce results
set.seed(123)
# Set-up k-fold cross-validation
trainSet <- trainControl(method = "cv", number = 10)
# Run linear regression model
model <- train(form = `Graduate Prospects 2018` ~ .,
...
# Section 4: Bayesian Multi-Linear Regression
```{r (HIDE) Bayesian Linear Regressions script, include = FALSE, echo = FALSE}
source("4 - analysisBayesian.R")
```
