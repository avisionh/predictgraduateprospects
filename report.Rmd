...
```{r (SHOW) Density and CDF plots, echo = TRUE, eval = TRUE}
...
(@) Our dependent variable, *Graduate Prospects 2017*, is the blue dot on the graph, and it is most closest to the triangle and asterisk objects, whilst also lying in the grey region. We will discuss each of these aspects in turn.
  ...
  $$ N(\mu,\: \sigma^2)\: where\: \mu\: \in\: \!R\: and\: \sigma^2\: >\: 0 $$
...
```{r (SHOW) Formulating priors, echo = TRUE, eval = TRUE}
# Graduate Prospects 2017 distribution
...
```{r (SHOW) Goodness-of-fit plots, echo = TRUE, eval = TRUE}
par(mfrow = c(2,2)) 
...
$$ Graduate\: Prospects\: \~\: N(\mu,\: \sigma^2) $$ 
...
```{r (SHOW) Traceplots, echo = TRUE, eval = TRUE}
par(mfrow = c(3,3))
...
```{r (SHOW) Accuracy, echo = TRUE, eval = TRUE}
...
(@) Write code to write *.csv* copy of web-scraped HTML table.
...
[^11]: We do not have a live link to an underlying database in this project, but this practice is included here for reference.
